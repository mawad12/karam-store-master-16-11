...
class BmoPageItemList: UIView, UICollectionViewDelegate, UICollectionViewDataSource, BmoPageItemListLayoutDelegate {
    ...
    func reloadData() {
        ...
        makeFull = bmoDataSource?.bmoViewSplitFull(in: bmoViewPager) ?? false
    ...
    func bmoPageItemListLayout(sizeForItemAt index: Int) -> CGSize {
        ...
        if(makeFull == true){
            let widthItem = (bmoViewPgaerNavigationBar?.frame.width)! / CGFloat(bmoViewPagerCount)
            return CGSize(width: widthItem, height: collectionView.bounds.size.height)
        }
        if navigationBar.orientation == .horizontal {
